...
- [x] 04-02-PLAN.md — Bounded graceful-shutdown drain, SHUTDOWN_GRACE_MS config, and Nest-hook Redis teardown on both entrypoints [ERR-05]
...
- [x] 06-01-PLAN.md — Bonus B: code-first GraphQL (MercuriusDriver) scan query + enqueueScan mutation delegating to ScanService, SSRF-parity, GraphiQL [API-01, API-02]
...
- [x] 06-03-PLAN.md — Static-serve the SPA from the API (route-exclude + boot-safe dist/web) and fold the web build into the Dockerfile/compose [FE-01, FE-02, FE-03]

**Wave 4** *(blocked on Wave 3 completion)*

- [ ] 06-04-PLAN.md — README (compose-first run guide + criterion #3 checklist) and ONBOARDING (What/Why/How + reviewer Q&A) [DOC-01, DOC-02]
...
| 6. Optional Bonuses & Documentation | 3/4 | In Progress|  |
