...
- **D-02:** The pipeline uses stream-json with **nested** descent so no `Result` is ever materialized as a whole object: `chain([fs.createReadStream(path), parser(), pick({ filter: 'Results' }), streamArray(), ...])` for the outer `Results` array, then a nested `pick`+`streamArray` over each Result's `Vulnerabilities` array. Filter `Severity === 'CRITICAL'` per vulnerability, map `TrivyVulnerability` → `Vulnerability` (camelCase, per Phase 1 D-04 shape), and `yield`. Backpressure propagates end-to-end via stream-chain. **Zero** `fs.readFile`/`readFileSync`/`JSON.parse`/`.toArray()`/`streamValues()` anywhere on the report path (ENGINE-05; PITFALLS Pitfall 1).
...
- **D-08:** A **mechanical forbidden-API guard** — a grep-based CI check (or custom ESLint rule) — asserts that `fs.readFile`/`readFileSync`, `JSON.parse`, `.toArray(`, and `streamValues(` never appear in the parser path (`apps/api/src/parser/**`). This turns ENGINE-05's prohibition into an enforced gate, not a promise (PITFALLS Pitfall 1 "warning signs").
