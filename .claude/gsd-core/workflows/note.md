...
- **Global scope**: `/home/dzmitry/Coding/oxtest/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/dzmitry/Coding/oxtest/.claude/notes/`)
...
2. Glob `/home/dzmitry/Coding/oxtest/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/dzmitry/Coding/oxtest/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/dzmitry/Coding/oxtest/.claude/notes/` — works in any directory
